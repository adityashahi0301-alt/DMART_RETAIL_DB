-- CREATING SCHEMA
CREATE SCHEMA RETAIL_DMART;

-- USE THE SCHEMA
USE RETAIL_DMART;
-- creating database
CREATE DATABASE RETAIL_DB;
-- DROPPING DATABAE - DONT RUN BCZ REMOVES THE ENTIRE DB AND STRUCTURE
DROP DATABASE RETAIL_DB;
-- USE THE DATABASE
USE  RETAIL_DB;
-- =================================================
-- CRETAING SALES_FACT TABLE
...
-- ================================================
-- create order-product-mapping table
...
-- =====================================================
--  creating the ‘branch_details’ table?
...
ORDER_PRODUCT_MAPPING(ORDER_ID);
-- =========================================================
-- adding the foreign key constraint to the ‘order_product_mapping’ table 
-- in order to connect it to the ‘product_info’ table?
...
-- ============================================================
-- adding the foreign key constraint to the ‘sales_fact’ table in order to connect it to the 
-- ‘customer_info’ table?
ALTER TABLE SALES_FACT
...
-- =====================================================
-- adding the foreign key constraint to the ‘sales_fact’ table in order 
-- to connect it to the ‘branch_details’ table?
ALTER TABLE SALES_FACT
ADD CONSTRAInt STORE_ID_FN
 FOREIGN KEY(STORE_ID) REFERENCES
branch_details(STORE_ID);
-- =================================================================
-- lOADING DATA INTO CUSTOMER INFO TABLE
...
-- =====================================================
-- LOADIN DATA IN TO BRANCH DETAILS TABLE
...
-- ===================================================
-- LOADING DATA INTO PRODUCT-INFO TABLE
...
-- Loadin data in to sales_fact table from excel
...
-- INSERT RECORDS IN TO ORDER_PRODCT_MAPPIN
...
-- What query will you write to find the number of transactions involving 
-- male customers?
SELECT 
    COUNT(s.order_id) AS male_transaction_count
FROM 
    sales_fact s
JOIN 
...
-- What query will you write to find the number of customers 
-- who placed an order from more than one store?

-- What query will you write to output the city with the highest number of orders
--  along with the number of orders?

SELECT B.CITY,COUNT(S.ORDER_ID) AS TOTAL_ORDERS FROM SALES_FACT S
JOIN BRANCH_DETAILS B ON B.STORE_ID=S.STORE_ID 
GROUP BY B.CITY ORDER BY TOTAL_ORDERS DESC LIMIT 1;

--
SELECT B.CITY,COUNT(S.ORDER_ID) AS TOTAL_ORDERS FROM SALES_FACT S
JOIN BRANCH_DETAILS B ON B.STORE_ID=S.STORE_ID 
GROUP BY B.CITY ORDER BY TOTAL_ORDERS DESC ;
--
-- What query will you write to output the area that recorded 
-- the highest sales in terms of sales amount, along with its sales amount?

SELECT B.AREA,COUNT(S.ORDER_ID) AS SALES FROM SALES_FACT S
JOIN BRANCH_DETAILS B ON B.STORE_ID=S.STORE_ID
GROUP BY B.AREA ORDER BY  SALES DESC;

SELECT B.AREA,SUM(OP.QUANTITY_ORDERED * P.SALE_PRICE) AS TOTAL_SALES_AMOUNT
 FROM SALES_FACT S
JOIN BRANCH_DETAILS B ON B.STORE_ID=S.STORE_ID
JOIN ORDER_PRODUCT_MAPPING OP ON OP.ORDER_ID=S.ORDER_ID
JOIN PRODUCT_INFO P ON P.PRODUCT_ID=OP.PRODUCT_ID
GROUP BY B.AREA ORDER BY  TOTAL_SALES_AMOUNT DESC LIMIT 1;

--
SELECT  b.area,SUM(m.quantity_ordered * p.sale_price) AS total_sales_amount
FROM     branch_details b
JOIN     sales_fact s ON b.store_id = s.store_id
JOIN     order_product_mapping m ON s.order_id = m.order_id
JOIN     product_info p ON m.product_id = p.product_id
GROUP BY     b.area ORDER BY total_sales_amount DESC;
--
-- What query will you write to find the state with the highest number of customers?
SELECT B.STATE,COUNT(S.CUSTOMER_ID) AS CUST_COUNT FROM SALES_FACT S
JOIN BRANCH_DETAILS B ON B.STORE_ID = S.STORE_ID
GROUP BY B.STATE ORDER BY CUST_COUNT DESC;
	
SELECT B.STATE,COUNT(DISTINCT S.CUSTOMER_ID) AS CUST_COUNT FROM SALES_FACT S
JOIN BRANCH_DETAILS B ON B.STORE_ID = S.STORE_ID
GROUP BY B.STATE ORDER BY CUST_COUNT DESC LIMIT 1;
